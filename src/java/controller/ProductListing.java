...
public class ProductListing extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!Validations.isInteger(categoryId)) {
            ...
            response_DTO.setContent("Please fill Prie");
            ...
            response_DTO.setContent("Pleas submit image 1");
        } else if (image2.getSubmittedFileName() == null) {
            response_DTO.setContent("Pleas submit image 2");
        } else if (image3.getSubmittedFileName() == null) {
            response_DTO.setContent("Pleas submit image 3");
        } else {
            Category category = (Category) session.get(Category.class, Integer.parseInt(categoryId));

            if (category == null) {
                response_DTO.setContent("Pleasw select a valid Category");
            } else {
                Model model = (Model) session.get(Model.class, Integer.parseInt(modelId));

                if (model == null) {
                    response_DTO.setContent("Pleasw select a valid Model");
                } else {
                    if (model.getCategory().getId() != category.getId()) {
                        ...
                        if (storage == null) {
                            response_DTO.setContent("Pleasw select a valid Storage");
                        } else {
                            Color color = (Color) session.get(Color.class, Integer.parseInt(colorId));

                            if (color == null) {
                                response_DTO.setContent("Pleasw select a valid Color");
                           } else {
                                    ...
                                    String newApplication = applicationPath.replace("build"+File.separator+"web", "web");
